...
fn main() {
    ...
    let asd = lines
        ...
        .map(|(l, v)| {
            ...
            let apa = vec![l.clone(), vec!["?".to_string()],l.clone(), vec!["?".to_string()],l.clone(), vec!["?".to_string()],l.clone(), vec!["?".to_string()],l.clone() ].iter().map(|f| f.clone()).flatten().collect::<Vec<String>>();
            let banan = vec![v.clone(),v.clone(),v.clone(),v.clone(),v.clone()].iter().flatten().map(|f| f.clone()).collect::<Vec<usize>>();
            let vv: Vec<String> = apa
                .iter()
                .skip_while(|s| **s == ".".to_string())
                .map(|x| x.clone())
                .collect();
            // println!("{:?}", vv);
            let a = search(&vv, &banan, 0, 0, 0);
            i+=1;
            println!("{} / {}", i, len);
            a
        })
